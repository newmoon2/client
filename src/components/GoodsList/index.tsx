    ...
    <div className={styles.goodsList}>
      {props.list.map((item) => {
        ...
        return (
          ...
          >
			<p className={styles.goodsList_item_level}>{ item.level === 1 ? '회원' : item.level === 2 ? '슈퍼멤버' : '슈퍼 특별 제안' } {item?.describe && <Popover content={item.describe} title={item.title}><QuestionCircleOutlined /></Popover>}</p>
